...
Praca ma na celu stworzenie zarysu środowiska w którym użytkownik ma możliwość wykorzystania algorytmów i narzędzi zaimplementowanych przez programistę w celu zwiększenia wydajności obliczeń które chce przeprowadzić.
Po przeprowadzeniu przeglądu dostępnych na rynku narzędzi stwierdzono brak narzędzi wypełniających niszę szeregowania serii bardzo długich operacji za pomocą metod udostępnianych przez matematyczne modele szeregowania zadań w systemie otwartym.
...
Przygotowano ideowy model działania systemu wraz z wytycznymi do jego implementacji.
Napisany został prototyp programu udowadniający możliwość stworzenia systemu który może zostać wykorzystany w przy różnych operacjach analitycznych w wielu środowiskach.
Zaimplementowano sprawny algorytm pozwalający na wykonanie uszeregowania.
Został wykonany przegląd literatury w tym temacie, ze szczególnym naciskiem na pracę wykorzystane przy w implementacji.
Przeanalizowano również jak wydajne pod kątem czasu przygotowania szeregowania są heurystyki opisane w literaturze.
Wskazane zostały potencjalne problemy mogące wystąpić przy działaniu systemu, a także zostały wymienione kierunki w których praca ta mogłaby się rozwijać.
\end{abstract}
...
Wiarygodne i uniwersalne oszacowanie czasów wykonywania różnych operacji nie jest możliwe. Rozważono wprowadzenie systemu szacowania czasu na podstawie analizy kilku (np. losowych, lub pierwszych) (zgrupowanych) wierszy algorytm oszacuje złożoność operacji na danym procesorze, a następnie porównanie rozmiaru wejścia z czasem, w którym jest wykonywany. Wykorzystując funkcję \emph{curve\_fit} z pakietu \emph{scipy.optimize} mogłyby zostać dobrane parametry $a$, $b$, $c$ do funkcji
...
W przypadku niezaimplementowania wymaganego przez użytkownika algorytmu szeregowania program zgłosi błąd. Dodanie nowego algorytmu wymaga wyłącznie na stworzeniu klasy implementującej funkcje klasy bazowej wbudowanej w system.
\medskip
...
W celu analizy sprawności algorytmu wstawianie z wiązką szukającą (\emph{insertion with beam search}) porównano sumę czasów działania dyspozytora oraz wykonywania zaszeregowania z sekwencyjnym czasem wykonywania operacji dla tych samych danych.

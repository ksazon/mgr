...
Posiadając ustaloną kolejność wstawiania operacji można przystąpić do~dokładania ich do kolejnych częściowych uszeregowań. Przy każdym wstawieniu operacji $O_{ij}$ rozważamy następujące możliwości nadania jej rangi $r_{ij}$:
\begin{enumerate}
    ...
    \item $r_{ij} = r_{kj} + 1$, gdzie $r_{kj}$ jest wartością w kolumnie $j$ częściowego uszeregowania, co interpretowane jest jako: operacja $O_{ij}$ jest bezpośrednim następcą operacji $O_{kj}$ na maszynie~$j$,
    \item $r_{ij} = r_{il} + 1$, gdzie $r_{il}$ jest wartością w wierszu $i$ częściowego uszeregowania, a maszyny $j$ i $l$ są ze sobą w konflikcie, co jest interpretowane jako: operacja $O_{ij}$ staje się bezpośrednim następcą operacji $O_{il}$ na~zadaniu~$i$.
\end{enumerate}
...
\begin{enumerate}
    ...
    $$\exists l \neq j, [r_{ij}=r_{lj} \textrm{ oraz } O_{ij} \textrm{ jest w konflikcie z } O_{il}]$$
\end{enumerate}
...
$\{M1, M2, M3, M4\}$, czterema zadaniami $\{J1, J2, J3, J4\}$ oraz zadanymi czasami przetwarzania, jak w tabeli \ref{tab:example_pt}.
Niech ponadto wykonywanie tego samego zadania na maszynach $M1$ i $M4$ jednocześnie będzie niedozwolone.
Algorytm zatrzymano w momencie, gdy do jednego z częściowych uszeregowań - kandydatów, wstawione zostały operacje widocznie na diagramie \ref{diag:state0}, a kolejną operacją do wstawienia jest $O_{J2M4}$.\\
...
$J4$ & 3 & 2 & 3 & 4 \\ \hline
\end{tabular}
\caption{Czasy przetwarzania dla przykładu działania algorytmu wstawiania\label{tab:example_pt}}
\source{Opracowanie własne}
\end{table}\medskip
...
\caption{Częściowe uszeregowanie przed wstawieniem operacji $O_{J2M4}$\label{diag:state0}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Wariant 1. nadania rangi: nadanie rangi $1$ - uszeregowanie jest~niepoprawne przez tę samą rangę dla $r_{J2M4}$ i $r_{J4M4}$\label{diag:state1_0}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Inkrementacja rangi $r_{J4M4}$ - uszeregowanie jest niepoprawne przez tę samą rangę dla $r_{J4M4}$ i $r_{J1M4}$\label{diag:state1_1}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Inkrementacja rangi $r_{J1M4}$ - uszeregowanie jest poprawne, a~najdłuższa ścieżka przechodząca przez wstawianą operację ma długość $6 + 4 + 5 = 15$, lub $10 + 5 = 15$\label{diag:state1}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Wariant 2. nadania rangi: nadanie rangi o $1$ większej niż $r_{J1M4}$ - uszeregowanie jest niepoprawne przez tę samą rangę dla $r_{J2M4}$ i $r_{J2M1}$\label{diag:state3_1}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Inkrementacja rangi $r_{J2M1}$ - uszeregowanie jest poprawne, a~najdłuższa ścieżka przechodząca przez wstawianą operację ma długość $10 + 5 + 6 + 4 = 25$\label{diag:state2}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Wariant 2. nadania rangi: nadanie rangi o $1$ większej niż $r_{J4M4}$ - uszeregowanie jest niepoprawne przez tę samą rangę dla $r_{J2M4}$ i $r_{J1M4}$\label{diag:state3_0}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\centering
\includegraphics[width=.7\hsize]{fig/3_1.png}
\caption{Inkrementacja rangi $r_{J1M4}$ - uszeregowanie jest niepoprawne przez tę samą rangę dla $r_{J2M1}$ i $r_{J2M4}$\label{diag:state3_1}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Inkrementacja rangi $r_{J2M1}$ - uszeregowanie jest poprawne, a~najdłuższa ścieżka przechodząca przez wstawianą operację ma długość $10 + 5 + 6 + 4 = 25$\label{diag:state3}}
\source{Opracowanie własne}
\end{figure}\medskip
...
\caption{Wariant 3. nadania rangi: nadanie rangi o $1$ większej niż~$r_{J2M1}$ - uszeregowanie jest poprawne, a najdłuższa ścieżka przechodząca przez~wstawianą operację ma długość $10 + 7 + 4 + 6 = 27$\label{diag:state4}}
\source{Opracowanie własne}
\end{figure}\medskip
